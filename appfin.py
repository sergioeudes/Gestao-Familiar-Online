...
def senha_configurada():
    """Lê a senha em st.secrets (Streamlit Cloud ou .streamlit/secrets.toml local) ou,
    se não houver, na variável de ambiente APP_PASSWORD (útil fora do Streamlit Cloud)."""
    try:
...
def exigir_login():
    ...
    Sem NENHUMA senha configurada (ambiente de desenvolvimento local), libera o acesso com um aviso,
    para não travar quem está só testando o código na própria máquina."""
    if st.session_state.get("autenticado"):
        return True
    correta = senha_configurada()
    if not correta:
        st.warning("⚠️ Nenhuma senha configurada (`app_password` em secrets.toml, ou variável de "
                   "ambiente `APP_PASSWORD`) — acesso liberado para desenvolvimento local.")
        return True

    _, meio, _ = st.columns([1, 2, 1])
    with meio:
        st.markdown("### 🔒 Gestão Financeira Familiar")
        st.caption("Acesso restrito à família.")
        with st.form("form_login"):
            senha = st.text_input("Senha:", type="password")
            entrar = st.form_submit_button("Entrar", width="stretch")
        if entrar:
            if rg.senha_confere(senha, correta):
                ...
                time.sleep(min(tentativas, 3))          # atraso crescente contra tentativas repetidas
