...
# ---------------------------------------------------------
def senha_confere(informada, correta):
    """Compara em tempo constante (contra timing attack). False se não houver senha configurada
    ou se a informada vier vazia — nunca deixa passar por comparação com valor vazio/None."""
    if not correta or not informada:
        return False
    return hmac.compare_digest(str(informada), str(correta))


# ---------------------------------------------------------
# Score de saúde financeira (0-100)
# ---------------------------------------------------------
def pontuacao_saude(*, saldo_contas, atraso, entradas_mes, saidas_mes, limite_usado_pct=None):
    """Nota de 0 a 100 com base em quatro sinais simples (todos os valores em centavos,
    exceto limite_usado_pct que é 0-100 ou None quando não há cartão). Não é um índice
    técnico de crédito, é só um resumo visual rápido da situação do mês.

    Descontos:
      saldo negativo........... -25
      algo em atraso............ -25 (mais -15 se o atraso for >= metade das entradas do mês)
      gastou mais do que entrou. -20
      limite de cartão > 80%.... -15
    """
    pontos = 100
    motivos = []
    ...
    if saldo_contas < 0:
        pontos -= 25
        motivos.append("saldo negativo")
    if atraso > 0:
        pontos -= 25
        motivos.append("contas em atraso")
        if entradas_mes > 0 and atraso >= entradas_mes / 2:
            pontos -= 15
            motivos.append("atraso alto em relação à renda do mês")
    if entradas_mes > 0 and saidas_mes > entradas_mes:
        pontos -= 20
        motivos.append("gastou mais do que entrou este mês")
    if limite_usado_pct is not None and limite_usado_pct > 80:
        pontos -= 15
        motivos.append("limite do cartão quase no fim")
    ...
    if pontos >= 80:
        ...
        status = "Alerta"
    return {"pontos": pontos, "status": status, "motivos": motivos}
