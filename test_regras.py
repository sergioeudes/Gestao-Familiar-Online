...
from regras import (agrupar_top_n, competencia_do_vencimento, dividir_em_parcelas, formatar_brl, gerar_vencimentos_fatura,
                    ...
                    pontuacao_saude, reais_para_centavos, senha_confere, status_fatura,
                    validar_cpf, variacao_percentual, vencimento_da_competencia, vencimento_da_fatura,
                    vencimento_no_mes)
...
# ---------- Senha de acesso ----------
def test_senha_confere():
    assert senha_confere("abc123", "abc123")
    assert not senha_confere("errada", "abc123")
    assert not senha_confere("", "abc123")          # informada vazia
    assert not senha_confere("abc123", "")           # nenhuma senha configurada
    assert not senha_confere("abc123", None)
    assert not senha_confere(None, "abc123")
    assert not senha_confere("ABC123", "abc123")     # sensível a maiúsculas/minúsculas


# ---------- Score de saúde financeira ----------
def test_pontuacao_saude_tudo_ok():
    r = pontuacao_saude(saldo_contas=100000, atraso=0, entradas_mes=500000, saidas_mes=300000)
    assert r == {"pontos": 100, "status": "Saudável", "motivos": []}


def test_pontuacao_saude_saldo_negativo():
    r = pontuacao_saude(saldo_contas=-100, atraso=0, entradas_mes=500000, saidas_mes=300000)
    assert r["pontos"] == 75 and "saldo negativo" in r["motivos"]


def test_pontuacao_saude_atraso_alto_desconta_duas_vezes():
    r = pontuacao_saude(saldo_contas=100000, atraso=300000, entradas_mes=500000, saidas_mes=100000)
    assert r["pontos"] == 60  # -25 (atraso) -15 (atraso >= metade da renda)
    assert "contas em atraso" in r["motivos"] and "atraso alto em relação à renda do mês" in r["motivos"]


def test_pontuacao_saude_gastou_mais_que_ganhou():
    r = pontuacao_saude(saldo_contas=100000, atraso=0, entradas_mes=100000, saidas_mes=200000)
    assert r["pontos"] == 80 and r["status"] == "Saudável"  # 80 ainda é o limite "Saudável"


def test_pontuacao_saude_limite_do_cartao_no_fim():
    r = pontuacao_saude(saldo_contas=100000, atraso=0, entradas_mes=100000, saidas_mes=50000, limite_usado_pct=95)
    assert r["pontos"] == 85
    r2 = pontuacao_saude(saldo_contas=100000, atraso=0, entradas_mes=100000, saidas_mes=50000, limite_usado_pct=50)
    assert r2["pontos"] == 100  # abaixo de 80% não desconta


def test_pontuacao_saude_nunca_fica_negativa():
    r = pontuacao_saude(saldo_contas=-1, atraso=999999, entradas_mes=1, saidas_mes=999999, limite_usado_pct=99)
    assert r["pontos"] == 0 and r["status"] == "Alerta"


def test_pontuacao_saude_status_por_faixa():
    assert pontuacao_saude(saldo_contas=1, atraso=0, entradas_mes=1, saidas_mes=0)["status"] == "Saudável"
    assert pontuacao_saude(saldo_contas=-1, atraso=1, entradas_mes=100, saidas_mes=0)["status"] == "Atenção"
    assert pontuacao_saude(saldo_contas=-1, atraso=100, entradas_mes=100, saidas_mes=200)["status"] == "Alerta"
