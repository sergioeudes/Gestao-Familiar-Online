...
def test_kpi_escapa_html_e_aplica_tom():
    ...
    assert "color:#f87171" in h                                           # valor grande colorido (vermelho do tema escuro)
...
def test_tabela_formata_em_padrao_brasileiro():
    ...
    assert "12,3%" in html
    assert "color: #f87171" in html and "color: #4ade80" in html  # vermelho/verde aplicados (tons claros do tema escuro)
...
def test_css_forca_barra_de_navegacao_horizontal():
    """Sem isso, as colunas do Streamlit empilham em telas estreitas e a barra de
    navegação vira uma pilha de botões cobrindo a tela no celular."""
    assert ".st-key-barra_nav" in vz.CSS
    assert "flex-direction: row" in vz.CSS
    for seletor in ('[data-testid="stHorizontalBlock"]', ".stHorizontalBlock",
                    '[data-testid="column"]', ".stColumn"):
        assert seletor in vz.CSS, seletor


def test_css_sem_linhas_em_branco():
    assert "\n\n" not in vz.CSS and vz.CSS.startswith("<style>") and vz.CSS.endswith("</style>")


def test_todo_cartao_grande_tem_cor_explicita():
    """Sem isso, um tom não listado em CLASSE_TEXTO cai silenciosamente sem cor e o texto
    fica apagado no tema escuro (foi o que aconteceu com o cartão de Patrimônio)."""
    for tom in ("positivo", "negativo", "alerta", "info", "neutro", "gradiente"):
        h = vz.kpi("Rótulo", "R$ 1,00", tom=tom, grande=True)
        assert 'style="color:' in h, f"tom={tom} sem cor explícita"
    assert 'style="color:#ffffff"' in vz.kpi("x", "y", tom="gradiente", grande=True)
